...
- 测试是为了最小化问题
- 测试首先在思维上转变，测试先行，做之前要先想好，不要盲目的写
- 减少重复，提交代码前本地测试必须通过，然后有ci来做更多的测试集成，当用例足够多的时候，测试的价值会极其明显
...
其实，如果大家熟悉rails框架，里面有一个migrate机制，其实做的就这样的事儿。不过它的意思更专注于数据库的数据和结构迁移罢了。